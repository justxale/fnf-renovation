 ...
 */
class DialAlphabet extends FlxSpriteGroup
{
	...
	var lastSprite:DialAlphaCharacter;
	var xPosResetted:Bool = false;

	var splitWords:Array<String> = [];

	public var isBold:Bool = false;
	public var lettersArray:Array<DialAlphaCharacter> = [];
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			var isNumber:Bool = DialAlphaCharacter.numbers.indexOf(character) != -1;
			var isSymbol:Bool = DialAlphaCharacter.symbols.indexOf(character) != -1;
			var isAlphabet:Bool = DialAlphaCharacter.alphabet.indexOf(character.toLowerCase()) != -1;
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:DialAlphaCharacter = new DialAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:DialAlphaCharacter = new DialAlphaCharacter(xPos, 0, textSize);
	...
	public function timerCheck(?tmr:FlxTimer = null) {
		...
		if(loopNum <= splitWords.length && splitWords[loopNum] != null) {
			...
			var isNumber:Bool = DialAlphaCharacter.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = DialAlphaCharacter.symbols.indexOf(splitWords[loopNum]) != -1;
			var isAlphabet:Bool = DialAlphaCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1;
			...
			if ((isAlphabet || isSymbol || isNumber) && (!isBold || !spaceChar))
			{
				...
				// var letter:DialAlphaCharacter = new DialAlphaCharacter(30 * loopNum, 0, textSize);
				var letter:DialAlphaCharacter = new DialAlphaCharacter(xPos, 55 * yMulti, textSize);
				letter.row = curRow;
				if (isBold)
				{
					if (isNumber)
...
class DialAlphaCharacter extends FlxSprite
{
	...
	{
		super(x, y);
		var tex = Paths.getSparrowAtlas('dialAlphabet');
		frames = tex;
